...
const AvatarDisplay = ({ onStartInterview }) => {
  return (
    <div className="flex flex-col items-center justify-center p-6 bg-slate-50 rounded-xl shadow-lg max-w-md mx-auto border border-slate-100">
      ...
      <div className="relative mb-4 group">
        ...
        />
        <div 
          className="absolute bottom-2 right-2 w-4 h-4 bg-green-500 border-2 border-white rounded-full animate-pulse" 
          title="Active">
        </div>
      </div>
      ...
      <div className="mt-6 flex space-x-3 w-full">
        <button 
          ...
          className="flex-1 py-2 px-4 bg-indigo-600 hover:bg-indigo-700 text-white font-medium rounded-lg shadow-sm transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2"
        >
          Start Interview
        </button>
        
        <button 
          className="py-2 px-4 bg-white border border-slate-200 text-slate-700 hover:bg-slate-50 font-medium rounded-lg shadow-sm transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-slate-400 focus:ring-offset-2"
        >
          Settings
        </button>
      </div>
